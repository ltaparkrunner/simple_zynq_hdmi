...
s32 offset)
{
	u32 bufferSize = dispHSize * dispVSize * 3;
	...
	for(i1=0; i1<vOffset; i1++)
	{
		for(i2=0; i2<(dispHSize/3); i2++)
		{
			videoFramePointer[((i1*dispHSize*3) + (i2*3)+ offset)%bufferSize]	= 0x00;
			videoFramePointer[((i1*dispHSize*3) + (i2*3) + 1+ offset)%bufferSize]	= (variant == 1)? 0xcc : 0x00;
			videoFramePointer[((i1*dispHSize*3) + (i2*3) + 2+ offset)%bufferSize]	= (variant == 1)? 0x00 : 0xcc;
		}
		for(i2=dispHSize/3; i2<(2*dispHSize/3); i2++)
		{
			videoFramePointer[((i1*dispHSize*3) + (i2*3)+ offset)%bufferSize]	= 0xee;
			videoFramePointer[((i1*dispHSize*3) + (i2*3) + 1+ offset)%bufferSize]	= 0x00;
			videoFramePointer[((i1*dispHSize*3) + (i2*3) + 2+ offset)%bufferSize]	= (variant == 1)? 0xcc : 0x00;

		}
		for(i2=2*dispHSize/3; i2<dispHSize; i2++)
		{
			videoFramePointer[((i1*dispHSize*3) + (i2*3)+ offset)%bufferSize]	= 0x00;
			videoFramePointer[((i1*dispHSize*3) + (i2*3) + 1+ offset)%bufferSize]	= (variant == 1)? 0x00 : 0xdd;
			videoFramePointer[((i1*dispHSize*3) + (i2*3) + 2+ offset)%bufferSize]	= (variant == 1)? 0xdd : 0x00;
		}
	}
	for(i1=vOffset; i1<vOffset+imageVSize; i1++)
	{
		...
		for(i2=hOffset; i2<hOffset+imageHSize; i2++)
		{
			videoFramePointer[((i1*dispHSize*3) + (i2*3)+ offset)%bufferSize]	= (variant == 1)? *imagePointer : *imagePointer;//*imagePointer/16;//(variant == 1)? *imagePointer/16 : 0xcc;
			videoFramePointer[((i1*dispHSize*3) + (i2*3) + 1+ offset)%bufferSize]	= (variant == 1)? *imagePointer : *imagePointer/2;//*imagePointer/16;
			videoFramePointer[((i1*dispHSize*3) + (i2*3) + 2+ offset)%bufferSize]	= (variant == 1)? *imagePointer/2 : *imagePointer;//*imagePointer/16;//(variant == 1)? *imagePointer/16 : 0x00;
			imagePointer++;
		}
		for(i2=hOffset+imageHSize; i2<dispHSize; i2++)
		{
